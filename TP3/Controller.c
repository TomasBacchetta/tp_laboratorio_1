...
/** \brief devuelve el id del ultimo valor de un archivo de texto o en su defecto binario
 *
 * \param path char*
...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
    ...
    Employee* nuevoEmpleado = employee_new();
     ...
     if (pArrayListEmployee != NULL && nuevoEmpleado != NULL){
        ...
        //si la lista no esta vacia, busca el id mas alto de la linkedlist
        system("cls");
        if (!ll_isEmpty(pArrayListEmployee)){
            id = employee_SearchHighestId(pArrayListEmployee);
            printf("Segun los empleados cargados en sistema, el id mas alto es: %d.\n", id);
            id++;
            printf("Se le asignara a al nuevo empleado el id %d.", id);
        //si la lista esta vacia, va a buscar el mayor de los ultimos id del archivo de texto y binario (ya ordenados por id)
        } else {
            id = controller_loadToCheckLastId("data.csv", "data.bin", pArrayListEmployee);
            //si no encuentra id porque no existen los archivos o estan vacios
            if (id == -1){
            ...
            //si consigue id
                ...
                printf("Segun los empleados guardados en los archivos, el id mas alto es: %d.\n", id);
                id++;
            ...
            if (ll_sort(pArrayListEmployee, employee_CompareId,1) < 0){
                ...
                for (int x = 0; x < ll_len(pArrayListEmployee); x++){
                    *auxEmp = *((Employee *)ll_get(pArrayListEmployee, x));
                    if (employee_getId(auxEmp, &id) < 0 ||
                    employee_getNombre(auxEmp, nombre) < 0 ||
                    employee_getHorasTrabajadas(auxEmp, &horasTra) < 0 ||
                    employee_getSueldo(auxEmp, &sueldo) < 0){
                        allRight = -1;
                        break;
                    } else {
